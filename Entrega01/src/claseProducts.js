...
class ProductManager {
    ...
    getProductoById = async (id) => {
        ...
        const productDB = this.productos.find(producto => producto.id === parseInt(id))
        if (!productDB) {
    ...
    updateProducto = async(id, producto) => {
        this.productos =  await this.getProductos()
        const indexProductDB = this.productos.findIndex(product => product.id === parseInt(id))
        if (indexProductDB === -1) {
            return "No existe producto"
        }
        const productoActualizado = {...this.productos[indexProductDB],...producto}
        this.productos[indexProductDB] = productoActualizado
        this.guardarCambios()
    }

    // Este elimina definitivamente de la base de datos
    ...
    // Este cambia a estado eliminado
    deleteProducto = async (id) => {
        this.productos = await this.getProductos()
        const indexProductDB = this.productos.findIndex(product => product.id === parseInt(id))
        if (indexProductDB === -1) {
        ...
        console.log("--------------------------------")
        this.productos[indexProductDB].status = false
        this.guardarCambios()
