...
//vocabulario pos: posicion
app.get("/api/palabras/:pos", (req, res) => {
    const pos = req.params.pos;
    if (isNaN(pos)) return res.status(404).send({ status: "error", error: "POS debe ser un numero" })
    const parsedPos = parseInt(pos)
    const palabras = frase.split(' ')
    if (parsedPos <= 0 || parsedPos > palabras.length) return res.status(400).send({ status: "error", error: "Posicion fuera del rango de la frase" })
...
app.put("/api/palabras/:pos", (req, res) => {
    const pos = req.params.pos
    const palabra = req.body.palabra
    if (isNaN(pos)) return res.status(404).send({ status: "error", error: "Posicion deber ser un numero" })
    const parsedPos = parseInt(pos)
    const palabras = frase.split(' ')
    if (parsedPos <= 0 || parsedPos > palabras.length) return res.status(404).send({ status: "error", error: "Posicion fuera del rango de la frase" })
